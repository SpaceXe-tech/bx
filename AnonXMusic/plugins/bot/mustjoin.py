...
def is_sudo(user_id: int) -> bool:
    try:
        return user_id in sudoers_list
    except TypeError:
        return False
...
async def check_user_membership(client: Client, user_id: int, chat_id: str) -> bool:
    try:
        ...
        print(f"⚠️ Bot is not admin in: {chat_id}")
        return True
    except ChannelPrivate:
        print(f"⚠️ Channel/Group is private or inaccessible: {chat_id}")
        return True
...
async def send_force_join_message(client: Client, user_id: int, old_msg_id: int = None):
    ...
    if need_channel or need_group:
        ...
        keyboard.append(
            [InlineKeyboardButton("🔄", callback_data="check_joined")]
        )
