...
from AnonXMusic.utils.thumbnails import get_thumb, get_qthumb
...
async def stream(
    ...
    if streamtype == "playlist":
        ...
        if await is_active_chat(chat_id):
            ...
            qthumb = await get_qthumb(vidid)
            await app.send_photo(original_chat_id, photo=qthumb, caption=_["queue_4"].format(position, title[:27], duration_min, user_name), reply_markup=InlineKeyboardMarkup(button))
        else:
            if not forceplay:
                db[chat_id] = []
            await Anony.join_call(chat_id, original_chat_id, file_path, video=status, image=thumbnail)
